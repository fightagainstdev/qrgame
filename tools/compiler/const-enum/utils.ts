...
export function evaluate(expr: ts.Expression, known: Map<string, number | string>): string | number {
...
export function hasModifier(node: ts.Node, modifier: ts.SyntaxKind) {
  return ts.canHaveModifiers(node) && ts.getModifiers(node)?.some((mod: ts.Modifier) => mod.kind === modifier);
}
